    ...
    <div class="detail-action">
      ...
      <div class="btn">
        ...
        <template v-if="order.orderState === 2">
            <!-- <XtxButton type="gray" size="small">再次购买</XtxButton> -->
            <XtxButton @click="$router.push(`/member/checkout?orderId=${order.id}`)"
            type="plain" size="small">再次购买</XtxButton>
        </template>
        <!-- 待收货 -->
...
import { useCancel, useConfirmOrder } from '../index'
export default {
  ...
  setup () {
    // 按照订单状态显示图标
    return { orderStatus, ...useCancel(), ...useConfirmOrder() }
  }
}
</script>
